...
public class CycleDetection {
   ...
   * connected/not strongly connected) referenced by <code>digraph</code>. Random directed edges
  ...
  /**
   * A DFS method to detect the existence of a cycle in the directed graph. Random directed edges
   * will be generated until a cycle is found. This should be called repeatedly by <code>
   * start(AdjacencyMapDigraph, SmartGraphPanel)</code>.
   *
   * @param digraph Directed graph
  ...
  private static void checkCycle(
      ...
      LinkedList<Vertex<String>> vertices,
      Set<Vertex<String>> visitedVertices,
      Set<Vertex<String>> onStackVertices,
      Map<Vertex<String>, Vertex<String>> parentsOfVertices,
      LinkedList<Set<Vertex<String>>> foundCycles,
      StringBuilder buf,
    ...
    for (Edge<Integer, String> edge : digraph.outgoingEdges(startVertex)) {
      ...
      if (!visitedVertices.contains(vertex)) {
        ...
        checkCycle(
            ...
            foundCycles,
            buf,
            graphView); // recursively perform DFS search
        ...
        for (vertexInCycle = startVertex;
            !vertexInCycle.equals(vertex);
            vertexInCycle = parentsOfVertices.get(vertexInCycle)) {
