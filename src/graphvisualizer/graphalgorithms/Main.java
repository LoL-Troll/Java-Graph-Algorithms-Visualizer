...
public class Main extends Application {
  ...
  public void start(Stage ignored) {
        ...
        graphVertex -> {
          switch (selectedVerticesCount) {
            case 0:
              ...
              menu.getStatusBox()
                  .appendText(
                      graphVertex.getUnderlyingVertex() + " is selected as starting vertex.\n\n");
              break;
            ...
            case 1:
              if (!graphVertex.getUnderlyingVertex().equals(startVertex)) {
                ...
                menu.getStatusBox()
                    .appendText(
                        graphVertex.getUnderlyingVertex() + " is selected as ending vertex.\n\n");
              }
              break;

            case 2:
              if (!graphVertex.getUnderlyingVertex().equals(startVertex)
                  && !graphVertex.getUnderlyingVertex().equals(endVertex)) {
                graphView.getStylableVertex(startVertex).setStyleClass("vertex");
                startVertex = endVertex;
                endVertex = graphVertex.getUnderlyingVertex();
                graphVertex.setStyleClass("selectedVertex");
                menu.getStatusBox()
                    .appendText(startVertex + " is selected as new starting vertex.\n\n");
                menu.getStatusBox()
                    .appendText(endVertex + " is selected as new ending vertex.\n\n");
              }
            default:
          }
        });
    ...
    graphView.setEdgeDoubleClickAction(
        graphEdge ->
            menu.getStatusBox().appendText(graphEdge.getUnderlyingEdge() + " is selected.\n\n"));
    ...
    menu.setCycleDetectionButtonAction(
        event -> {
          for (Vertex<String> vertex : defaultDigraph.vertices()) {
            graphView.getStylableVertex(vertex).setStyleClass("vertex");
          }
    ...
    menu.setShortestPathButtonAction(
        event -> {
          if (startVertex != null && endVertex != null) {
            menu.getStatusBox()
                .appendText(ShortestPath.start(defaultDigraph, graphView, startVertex, endVertex));
            selectedVerticesCount = 0;
            startVertex = endVertex = null;
          } else {
            menu.getStatusBox()
                .appendText(
                    "Please select two vertices to compute the shortest path between them.\n\n");
          }
        });

    menu.setAddVertexButtonAction(
        event -> {
  ...
  private void resetDefaultDigraph(
      Graph<String, Integer> defaultDigraph, SmartGraphPanel<String, Integer> graphView) {
