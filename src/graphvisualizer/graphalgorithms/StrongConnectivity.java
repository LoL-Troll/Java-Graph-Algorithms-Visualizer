...
/**
 * This class is used to determine the strong connectivity of a directed graph. This method {@link
 * #checkStronglyConnected(AdjacencyMapDigraph, Vertex, SmartGraphPanel)} implements DFS to
 * determine strong connectivity. New edges are generated when the graph is not strongly connected
...
public class StrongConnectivity {
  ...
  /**
   * Generate a visualization for the DFS algorithm for a directed graph. This method calls the DFS
   * algorithm {@link #checkStronglyConnected(AdjacencyMapDigraph, Vertex, SmartGraphPanel)} to
   * determine the connectivity. This method will generate a new edge when the graph is not strongly
   * connected.
   *
   * @param digraph Directed graph
   * @param graphView Graph visualization object
   * @return description of edges that are newly added, result of the DFS algorithm {@link
   *     #checkStronglyConnected(AdjacencyMapDigraph, Vertex, SmartGraphPanel)}
   */
  public static String start(
      AdjacencyMapDigraph<String, Integer> digraph, SmartGraphPanel<String, Integer> graphView) {
  ...
  /**
   * This method is a DFS algorithm used to determine strong connectivity of a directed graph. This
   * method is repeatedly called by {@link #start(AdjacencyMapDigraph, SmartGraphPanel)}
   *
   * @param digraph Directed graph
  ...
  private static void checkStronglyConnected(
      AdjacencyMapDigraph<String, Integer> digraph,
    ...
    for (Edge<Integer, String> edge : digraph.outgoingEdges(startVertex)) {
      ...
      if (ids[ref.get(vertex)] == UNVISITED) checkStronglyConnected(digraph, vertex, graphView);
  ...
  /**
   * This method is used to initialize the hashmap named 'ref'. The hashmap is used as a index
   * reference of the vertex when accessing to arrays.
   *
   * @param digraph Directed graph
   * @param ref A hashmap that stores the vertex as key and Integer as value.
   */
  public static void setRef(
      AdjacencyMapDigraph<String, Integer> digraph, HashMap<Vertex<String>, Integer> ref) {
    verticesList = new LinkedList<>(digraph.vertices());
    int count = 0;
  ...
  /**
   * Initialize the members of the class before {@link #checkStronglyConnected(AdjacencyMapDigraph,
   * Vertex, SmartGraphPanel)} is executed. The members can be found declared as global variables of
   * this class.
   *
   * @param digraph Directed graph
   */
